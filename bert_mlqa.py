...
inputs,sid,start,end = get_data("parab","quesb","asnrb")
...
#         x = x.view(1,inp_len,self.hidden_size)
        ...
        c0 = torch.zeros(self.layer_dim*2, self.batch_size, self.hidden_dim).requires_grad_().to(device)

        out, (hn, cn) = self.lstm(seq, (h0.detach(), c0.detach()))
        out = out.to(device)
        # print(out.shape)
        out = self.highway(out[0]).to(device)
        # print(out.shape)
        out = out.unsqueeze(0).to(device)
        # print(out.shape)
        h1 = torch.zeros(self.layer_dim*2, self.batch_size, self.hidden_dim).requires_grad_().to(device)
        c1 = torch.zeros(self.layer_dim*2, self.batch_size, self.hidden_dim).requires_grad_().to(device)
...
for epoch in range(30):
    train(model,inputs[:350],sid[:350],optimizer,start[:350],end[:350])
test(model,inputs[350:],sid[350:],start[350:],end[350:])

torch.save(model,'mlqamodel')


# m = torch.load('mlqamodel')
# m = m.to(device)
